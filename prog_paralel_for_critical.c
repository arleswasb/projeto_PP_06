...
void pi_paralel_for_critical() {
    ...
    int thread_id = omp_get_thread_num();
        ...
        double x = (double)my_rand_r(&seeds[thread_id]) / RAND_MAX * 2.0 - 1.0;
        double y = (double)my_rand_r(&seeds[thread_id]) / RAND_MAX * 2.0 - 1.0;

        if (x * x + y * y < 1.0) {
            // CORREÇÃO 2: A diretiva #pragma omp critical
            // garante que apenas uma thread possa executar este bloco de código por vez.
            #pragma omp critical
            {
                pontos_no_circulo++;
            }
